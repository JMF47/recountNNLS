...
* `assays()` returns a list of length 2 that can be access with `$`
      + Using `$counts` returns a matrix of estimated transcript abundance on a number of reads scale
      + Using `$se` returns a matrix of estimated standard error of the abundance
